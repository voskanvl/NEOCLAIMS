...
const SortUp: FC<{ matched: boolean | undefined }> =
    ({ matched }) => <svg viewBox="0 0 7 4" width="7" height="4" fill={matched ? "#000" : "#C2C2C2"}>
        <use xlinkHref="/icon-sprite.svg#sortup"></use>
    </svg>
const SortDown: FC<{ matched: boolean | undefined }> =
    ({ matched }) => <svg viewBox="0 0 7 4" width="7" height="4" fill={matched ? "#000" : "#C2C2C2"}>
